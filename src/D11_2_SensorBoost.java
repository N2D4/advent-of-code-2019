...
@CacheVersion(0)
public class D11_2_SensorBoost extends AbstractSubmission {
    ...
    public void testCase() {
        try {
            ...
            colors.put(new IntIntPair(0, 0), ONE);
            int x = 0;
            int y = 0;
            ...
            int minX = colors.keySet().stream().mapToInt(a -> a.a).min().getAsInt();
            int maxX = colors.keySet().stream().mapToInt(a -> a.a).max().getAsInt();
            int minY = colors.keySet().stream().mapToInt(a -> a.b).min().getAsInt();
            int maxY = colors.keySet().stream().mapToInt(a -> a.b).max().getAsInt();

            for (int j = maxY; j >= minY; j--) {
                for (int i = minX; i <= maxX; i++) {
                    out.print(colors.getOrDefault(new IntIntPair(i, j), ZERO).equals(ONE) ? '#' : ' ');
                }
                out.println();
            }
    ...
    public static void main(String[] args) throws Exception {
        buildAndRun(D11_2_SensorBoost.class, System.getProperty("user.home") + "/Downloads", "unique_id_D2_1_1202ProgramAlert_02.12.19_15:11_Advent of Code");
    }

    public D11_2_SensorBoost() {
